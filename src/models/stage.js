class stage {
    ...
    getWeaponDrop(inRarity) {
        let EligibleWeapons = this.weaponDrops.filter(
            (weapon) => weapon.rarity == inRarity
        )
        return EligibleWeapons[
            Math.floor(Math.random() * EligibleWeapons.length)
        ]
    }

    getArmorDrop(inRarity) {
        let EligibleArmors = this.armorDrops.filter(
            (armor) => armor.rarity == inRarity
        )
        return EligibleArmors[Math.floor(Math.random() * EligibleArmors.length)]
    }

    getRecipeDrop(inRarity) {
        let EligibleRecipes = this.recipeDrops.filter(
            (recipe) => recipe.rarity == inRarity
        )
        return EligibleRecipes[
            Math.floor(Math.random() * EligibleRecipes.length)
        ]
    }

    getConsumableDrop(inRarity) {
        let EligibleConsumables = this.consumableDrops.filter(
            (consumable) => consumable.rarity == inRarity
        )
        return EligibleConsumables[
            Math.floor(Math.random() * EligibleConsumables.length)
        ]
    }

    getChestDrop(inRarity) {
        let EligibleChests = this.chestDrops.filter(
            (chest) => chest.rarity == inRarity
        )
        return EligibleChests[Math.floor(Math.random() * EligibleChests.length)]
    }

    getRawMaterialDrop(inRarity) {
        let EligibleRawMaterials = this.rawMaterialDrops.filter(
            (rawMaterial) => rawMaterial.rarity == inRarity
        )
        return EligibleRawMaterials[
            Math.floor(Math.random() * EligibleRawMaterials.length)
        ]
    }
